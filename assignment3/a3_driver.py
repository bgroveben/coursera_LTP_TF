...
def get_players_list():
    ...
    Prompt the player(s) to enter their names.
    Return a list of player info as a two-item list with name and score, respectively.
    """
    players = []
    ...
    while player.strip() or not players:
        ...
        if players:
            print("Leave a blank player name to begin playing, player.")
...
def play_game(players, board, words):
    ...
    while num_remaining > 0:
        ...
        guess = input("[{player}] Enter a word (or leave blank to pass): ".format(
                player=players[player_num % len(players)][0]))

        guess = guess.strip().upper()
        if a3.is_valid_word(words, guess) and a3.board_contains_word(board, guess) and not guess in found_words:
...
def print_headers(players, board, found_words, num_remaining):
    ...
    Display the score, board, and some other details.
    """
    print_score(players)
    print_board(board)
